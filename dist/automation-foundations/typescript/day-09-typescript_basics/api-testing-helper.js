...
function buildUrl(baseUrl, // API base URL (e.g., "https://api.example.com")
...
function simulateApiCall(endpoint, baseUrl) {
    // Log request details
...
function createValidationRule(name, // Descriptive name
validator, // Validation function
errorMessage // Error message if fails
) {
...
function validateResponse(response, // The response to validate
...
function runApiTest(endpoint, // Endpoint to test
baseUrl, // API base URL
validationRules // Validations to perform
) {
    ...
    const response = simulateApiCall(endpoint, baseUrl);
...
function generateTestReport(testResults) {
    ...
    const totalResponseTime = testResults.reduce((sum, result) => sum + result.response.responseTime, 0);
...
const getUserValidations = [
    ...
    createValidationRule("Status code should be 200", (response) => response.statusCode === 200, "Expected status code 200 but received " + getUserEndpoint.method),
    // Rule 2: Check response category is success
    createValidationRule("Response should be successful", (response) => response.statusCategory === "success", "Response status category should be 'success'"),
    // Rule 3: Check response time is acceptable
    createValidationRule("Response time should be under 1000ms", (response) => response.responseTime < 1000, "Response time exceeded acceptable limit"),
    // Rule 4: Check response has data
    createValidationRule("Response body should contain data", (response) => response.body && response.body.data, "Response body missing 'data' property")
];
// Run the test
...
const createUserValidations = [
    ...
    createValidationRule("Status code should be 201", (response) => response.statusCode === 201, "Expected status code 201 for resource creation"),
    // Rule 2: Check response contains success flag
    createValidationRule("Response should indicate success", (response) => response.body && response.body.success === true, "Response should have success: true"),
    // Rule 3: Check response contains created data with ID
    createValidationRule("Response should contain created resource ID", (response) => response.body && response.body.data && response.body.data.id, "Created resource should have an ID")
];
// Run the test
...
const deleteUserValidations = [
    ...
    createValidationRule("Status code should be 204", (response) => response.statusCode === 204, "Expected status code 204 for successful deletion"),
    // Rule 2: Check response time
    createValidationRule("Response time should be under 500ms", (response) => response.responseTime < 500, "Delete operation took too long")
];
// Run the test
...
const allResults = [test1Result, test2Result, test3Result];
