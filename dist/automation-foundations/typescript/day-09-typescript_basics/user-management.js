...
function createUser(id, // User ID - must be numeric
name, // Full name - must be string
email, // Email address - must be string
role, // Role - must be one of the UserRole values
status // Status - must be one of the AccountStatus values
) {
...
function generateUserReport(users) {
    ...
    }, {}); // Initial value: empty object typed as Record
...
const user1 = createUser(1, "Alice Johnson", "alice@test.com", "admin", "active");
const user2 = createUser(2, "Bob Smith", "bob@test.com", "tester", "active");
const user3 = createUser(3, "Charlie Brown", "charlie@test.com", "viewer", "inactive");
const user4 = createUser(4, "Diana Prince", "diana@test.com", "tester", "pending");
...
const loginResult1 = loginUser(validLogin);
...
const loginResult2 = loginUser(invalidLogin);
...
const testers = getUsersByRole(allUsers, "tester");
